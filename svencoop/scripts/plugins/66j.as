...
void checkEnoughPlayersFinished(CBasePlayer@ plr, bool printFinished=false)
{
	...
	if (level_change_triggered and (everyone_finish_triggered or !everyoneFinished))
	{
		...
		{
			if (everyoneFinished)
				g_PlayerFuncs.SayText(plr, "You reached the barrier.\n");
			else
				g_PlayerFuncs.SayText(plr, "You reached the barrier. " + needPlayers + " " + plrTxt + " needed to open barrier.\n");
		}
		return;
	}
	...
	if (printFinished)
	{
		if (everyoneFinished && percentage >= 100)
			msg = "" + plr.pev.netname + " reached the barrier. Everyone has finished now, touch the logo again to open. ";
		else if (isEnough)
			msg = "" + plr.pev.netname + " reached the barrier. Enough players have finished now, touch the logo again to open. ";
		else
			msg = "" + plr.pev.netname + " reached the barrier. ";
	}
	else
		msg = "" + percentage + "% reached the barrier. ";
	...
	if (isEnough or everyoneFinished)
	{
		...
		if (everyoneFinished)
		{
			...
			doCountdown("Barrier opening in ", 3, true);
			g_Scheduler.SetTimeout("triggerNextLevel", 3.0f, @plr);
		}
		else
		{
			doCountdown("Barrier opening in ", int(delay), false);
		...
		msg += "" + needPlayers + " " + plrTxt + " needed to open barrier.";	
...
void undo_teleport()
{
	...
	g_PlayerFuncs.SayTextAll(chatPlr, "Barrier trigger enabled. Touch the logo again to open the barrier.\n");
}
...
CBaseEntity@ getCaller(string name, bool recurse=true)
{
	...
	do {
		...
		{			
			if (ent2.pev.targetname == "trigger_percent10")
				continue;
...
void checkForChangelevel()
{
	...
	do {
		...
		if (ent is null)
			break;
		if (ent.pev.targetname != "trigger_percent10" and ent.pev.classname != "game_end")
			continue;

		if (ent.pev.targetname == "trigger_percent10" and ent.pev.solid == SOLID_BSP)
		...
		if (isTriggered and ent.pev.targetname == "trigger_percent10" and 
		...
		if ( args[0] == ".rush" )
		{
			...
			else
			{
				...
				if (g_timerMode.GetBool())
					msg = "Anti-rush is enabled for this map (" + neededPercent + "%). " + needPlayers + " " + plrTxt + " needed to open barrier.";
				else
