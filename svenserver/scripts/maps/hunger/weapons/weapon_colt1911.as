...
class weapon_colt1911 : ScriptBasePlayerWeaponEntity
{
	...
	bool Deploy()
	{
		bool bResult;
		{
			bResult = self.DefaultDeploy ( self.GetV_Model( COLT_V_MODEL ), self.GetP_Model( COLT_P_MODEL ), M1911A1_DRAW, "onehanded" );
	...
	void PrimaryAttack()
	{
		...
		else if( self.m_iClip > 0 )
		{
			...
			m_pPlayer.pev.waterlevel == WATERLEVEL_HEAD ? 
		...
		{
			case 0: g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_WEAPON, COLT_S_FIRE1, Math.RandomFloat( 0.95, 1.0 ), ATTN_NORM, 0, 93 + Math.RandomLong( 0, 0xf ) ); break;
			case 1: g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_WEAPON, COLT_S_FIRE2, Math.RandomFloat( 0.95, 1.0 ), ATTN_NORM, 0, 93 + Math.RandomLong( 0, 0xf ) ); break;
		}
		...
		m_pPlayer.FireBullets( 1, vecSrc, vecAiming, VECTOR_CONE_2DEGREES, ( m_pPlayer.pev.waterlevel == WATERLEVEL_HEAD ) ? 2048 : 8192, BULLET_PLAYER_CUSTOMDAMAGE, 4, m_iBulletDamage );
		...
		m_pPlayer.pev.punchangle.x += Math.RandomFloat( -1.9, -1.5 );
		...
		THGetDefaultShellInfo( m_pPlayer, vecShellVelocity, vecShellOrigin, 23, 7, -7 );
...
string COLTName()
{
	return "weapon_colt1911";
}

void RegisterCOLT()
{
	g_CustomEntityFuncs.RegisterCustomEntity( COLTName(), COLTName() );
	g_ItemRegistry.RegisterWeapon( COLTName(), "hunger/weapons", "9mm" );
}